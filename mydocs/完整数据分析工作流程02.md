...
### 1. [exploratory-data-analysis](skills/exploratory-data-analysis/SKILL.md) — 主打 EDA + 验证
- **方法**：UTF-8 矩形 schema/profile 审计、缺失值/泄漏审计、分布/异常值/变换敏感性分析、影像/序列元数据只读检查
- **工具/库**：`csv`/`json`（标准库）、`numpy 2.5.1`、`h5py 3.16.0`、`biopython 1.87`、`pillow 12.3.0`、`tifffile 2026.7.14`；`pandas 3.0.5` / `polars 1.43.0`（可选）
- **入口**：`scripts/capability_manifest.py`（能力清单）、`eda_analyzer.py`、`missingness_leakage_audit.py`、`distribution_sensitivity.py`、`tabular_profile.py`、`sequence_inspector.py`、`image_inspector.py`、`report_scaffold.py`
- **脚本内方法**：`analyze_file` / `build_report` / `_main`、IQR fences、MAD、winsorized mean、log diagnostics（脚本**只报告、不修改数据**）

### 2. [scikit-learn](skills/scikit-learn/SKILL.md) — ML 流水线中的 QC
- **方法**：Pipeline 内数据泄漏防护、训练/测试分层切分、交叉验证、缩放/编码仅在训练集 fit、随机种子固定、收敛/性能诊断
- **工具/库**：`scikit-learn` 的 `Pipeline` / `ColumnTransformer` / `cross_val_score` / `StratifiedKFold`
- **入口**：`scripts/classification_pipeline.py`（`create_preprocessing_pipeline`、`train_and_evaluate_model`）、`scripts/clustering_analysis.py`
- **关键约束**："Fit on Training Data Only"、"Use Stratified Splitting"、"Set Random State for Reproducibility"、"Scale Features When Required"

### 3. [statistical-analysis](skills/statistical-analysis/SKILL.md) — 假设与异常值检验
- **方法**：正态性检验、组间方差齐性检验、线性度检验、异常值检测、回归诊断、效应量 + 置信区间
- **工具/库**：`pingouin>=0.6`、`scipy>=1.11`、`statsmodels>=0.14.6`、`matplotlib`/`seaborn`
- **入口**：`scripts/assumption_checks.py`（`check_normality`、`check_normality_per_group`、`check_homogeneity_of_variance`、`check_linearity`、`detect_outliers`、`check_regression_diagnostics`、`comprehensive_assumption_check`）
- **贝叶斯 QC 替代**：`pymc>=5.0` + `arviz>=1.0`（`az.summary(..., ci_prob=0.95)`）

### 4. [analytical-method-validation](skills/analytical-method-validation/SKILL.md) — 分析方法验证（ICH Q2(R2)/Q14, USP, ICH M10, CLSI EP, ISO 17025）
- **方法**：线性/范围、检测限/定量限（LOD/LOQ）、准确度+精密度合并评估、特异性、稳健度、方法转移（Deming、Passing-Bablok、Bland-Altman、等效性检验）、OOS 调查
- **工具/库**：纯 Python 3.11 + 标准库（自实现统计分布，无 numpy/scipy）
- **入口**：`scripts/plan_validation.py`（设计验证计划）、`check_response.py`（响应/线性/范围）、`check_detection_limits.py`（LOD/LOQ）、`check_accuracy_precision.py`（A&P）、`check_bioanalytical_run.py`（ICH M10 ISR）、`compare_methods.py`（方法转移/比较）

### 5. [iso-standards-readiness](skills/iso-standards-readiness/SKILL.md) — ISO 体系下的数据/记录验证
- **方法**：文档与记录控制审计、供应商控制审计、追溯性审计、CAPA 审计、QMSR 转换审计、范围与证据清单校验
- **工具/库**：标准库 + 本地 JSON/Markdown
- **入口**：`scripts/audit_document_records.py`、`check_capa.py`、`check_supplier_controls.py`、`check_traceability.py`、`check_qmsr_transition.py`、`validate_evidence_manifest.py`、`validate_scope_intake.py`、`gap_analyzer.py`
- **覆盖标准**：ISO 13485:2016、ISO 14971:2019 + ISO/TR 24971:2020、ISO/IEC 17025:2017、ISO 15189:2022

---

## B. 领域（11 个 skill，按域排列）

### 组学 / NGS
### 6. [bulk-rnaseq](skills/bulk-rnaseq/SKILL.md) — 阶段化质控
- **方法**：样本表（samplesheet）预验证 → FastQC + MultiQC 原始 read QC → fastp/Trim Galore 修剪后再 QC → STAR/Salmon 映射率/重复率/rRNA/复杂度检查 → 计数后 PCA + 样本距离热图作 post-quant QC
- **工具/库**：`nf-core/rnaseq 3.26.0`（Nextflow）、`FastQC`、`MultiQC`、`fastp`、`Trim Galore`、`STAR 2.7.11b`、`Salmon 1.10.3`（decoy-aware）、`featureCounts (subread)`、`pytximport`
- **入口**：`scripts/validate_samplesheet.py`（`validate_samplesheet`、`validate_metadata`、`Report`）、`scripts/build_counts_matrix.py`
- **配套文档**：`references/design-and-qc.md`（实验设计与 QC 阈值解释）、`references/counts-and-handoff.md`、`references/upstream-nfcore.md`、`references/upstream-manual.md`

### 7. [scanpy](skills/scanpy/SKILL.md) — 单细胞 QC 在最前
- **方法**：细胞/基因过滤、线粒体比例（pct_counts_mt）阈值、Scrublet 双胞检测、HVG 选择、PCA 方差比、Leiden 聚类分辨率
- **工具/库**：`scanpy 1.12.x`（Python 3.12+）、`anndata ≥0.10`、`python-igraph`、`leidenalg`、`scrublet`、`dask`（可选）
- **入口脚本**：`scripts/qc_analysis.py`（`--scrublet`）、`scripts/preprocess.py`、`scripts/reduce_dimensions.py`、`scripts/cluster.py`；或 `scripts/run_pipeline.py` 一键
- **关键参数**：`min_genes`(200–500)、`min_cells`(3–10)、`pct_counts_mt`(5–20%)、`n_top_genes`(2000–3000)、`n_pcs`、`resolution`(0.4–1.2)

### 8. [deeptools](skills/deeptools/SKILL.md) — NGS QC + 可视化
- **方法**：BAM/bigWig/BED 文件格式与索引校验、相关性、PCA、fingerprint、覆盖度均一化、热图/剖面图
- **工具/库**：`deeptools 3.5.6`（conda/bioconda 推荐）；CLI 包括 `bamCoverage`、`bamCompare`、`plotFingerprint`、`plotCorrelation`、`plotPCA`、`computeMatrix`、`plotHeatmap`、`plotProfile`
- **入口**：`scripts/validate_files.py`（`check_file_exists`、`check_bam_index`、`check_bigwig_file`、`check_bed_file`、`validate_files`）、`scripts/workflow_generator.py`（ChIP-seq QC 流水线模板）

### 9. [bids](skills/bids/SKILL.md) — 神经影像数据规范验证
- **方法**：BIDS 规范校验、实体/命名/JSON sidecar 必填字段核查、`.bidsignore` 配置、MRIQC/fMRIPrep 衍生品 QC
- **工具/库**：`pybids`、`bids-validator-deno`（或 Deno `npm:bids-validator`）、`dcm2niix`、`MRIQC`、`fMRIPrep`、`QSIPrep`、`HeuDiConv`、`dcm2bids`、`BIDScoin`
- **入口**：`scripts/update_schema.py`（BIDS schema 同步）；引用文档 `references/core_workflows.md`（含 Validation 章节）
- **QC 输出**：BIDS-Apps 自动产生 BIDS 兼容的 `dataset_description.json` + 衍生品

### 10. [pathml](skills/pathml/SKILL.md) — 计算病理 QC
- **方法**：图像分块（tile）+ 组织检测（`TissueDetectionHE`）+ 掩膜 + 图像级 QC（亮度/对比度/焦点）、多路复用图谱/空间图谱 schema 校验
- **工具/库**：`pathml 3.0.5`（`HESlide`、`Pipeline` 含 `BoxBlur`、`TissueDetectionHE`）、`OpenSlide 1.3.1`、`python-bioformats 4.1.0`、`Torch 2.8.0`（可选）
- **入口**：`scripts/image_qc.py`（`read_image`、`synthetic_image`、`calculate_qc`、`_write_mask`、`build_parser`、`main`）、`scripts/validate_spatial_schema.py`（`validate_graph`、`validate_multiplex`）、`scripts/slide_manifest.py`、`scripts/plan_pipeline.py`、`scripts/plan_inference.py`

### 11. [lamindb](skills/lamindb/SKILL.md) — 数据湖仓 + 模式验证
- **方法**：Bionty 本体注释、灵活/最小/严格 schema、FAIR 合规校验、`ln.track()`/`ln.finish()`/`@ln.flow()` 血缘跟踪
- **工具/库**：`lamindb`、`bionty`（基因/蛋白/细胞类型/组织/疾病/通路本体）、`TileDB-SOMA`、`Zarr`、`Parquet`、`SQLite/Postgres`
- **入口**：无独立 scripts（能力在 `references/data-management.md`、`references/core-concepts.md` 中）；主入口是 `ln.track` / `Artifact.validation` / Bionty 查找

### 12. [neurokit2](skills/neurokit2/SKILL.md) — 生理信号预处理 + 验证
- **方法**：信号数据契约（采样率/单位/极性/缺口/饱和）、在线统计（`OnlineStats`）、CSV 探查、多模态 manifest 校验、EDA/ECG/HRV 流水线统计
- **工具/库**：`neurokit2 0.2.13`（Python 3.10+）、`numpy`、`scipy`、`pandas`、`scikit-learn`、`matplotlib`、`PyWavelets`
- **入口**：`scripts/inspect_signal.py`（`OnlineStats`、`_parse_units`、`inspect_csv`）、`scripts/eda_pipeline.py`（`_finite_values`、`_stats`）、`scripts/validate_multimodal.py`（`_stream_profile`、`validate_manifest`）、`scripts/ecg_hrv_pipeline.py`、`scripts/plan_epochs.py`

### 化学 / 分子 / 神经 / 行为
### 13. [flowio](skills/flowio/SKILL.md) — 流式细胞术数据 QC
- **方法**：FCS 文件元数据/门控/补偿解析，文件级验证
- **工具/库**：`flowio`
- **入口**：flowio 库自带读取与校验

### 14. [markitdown](skills/markitdown/SKILL.md) — 文档转结构化的 QC
- **方法**：多格式（PDF/Office/图像/音频）→ Markdown 转换的格式/编码校验
- **工具/库**：`markitdown` 系列解析器
- **入口**：markitdown CLI

### 15. [openpiv](skills/openpiv/SKILL.md) — 粒子图像测速 QC
- **方法**：图像对预处理、互相关、矢量场后处理 + 异常矢量过滤
- **工具/库**：`openpiv`
- **入口**：openpiv 库函数

### 16. [shap](skills/shap/SKILL.md) — 模型可解释性作为数据 QC
- **方法**：SHAP 值分布、特征依赖图、训练/测试分布漂移检测
- **工具/库**：`shap`
- **入口**：shap 库
...
| 场景                    | 推荐 skill 链路                                            |
| ----------------------- | ---------------------------------------------------------- |
| 通用 CSV/TSV 数据 QC    | `exploratory-data-analysis` → `statistical-analysis`       |
| 监管/合规 QC            | `iso-standards-readiness` → `analytical-method-validation` |
| Bulk RNA-seq 完整流水线 | `bulk-rnaseq` → `pydeseq2` → `pathway-enrichment`          |
| 单细胞                  | `scanpy`（QC 在 pipeline 最前段）                          |
| 神经影像                | `bids`（数据组织+验证）→ `deeptools`（NGS 比对/覆盖度 QC） |
| 病理                    | `pathml`（tile + image_qc）                                |
| 生理信号                | `neurokit2`（`inspect_signal` + `validate_multimodal`）    |
